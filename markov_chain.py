...
def Prefix_Suffix_Pairs(prefix_length, essay_list):
    ...
    for essay in essay_list:
        ...
        for i in range(len(essay_words) - prefix_length):
            ...
            if tuple(this_prefix) not in prefix_dict:
                prefix_dict[this_prefix] = {}
            if this_suffix not in prefix_dict[this_prefix]:
                prefix_dict[this_prefix][this_suffix] = 0
...
def Next_Word(prefix_dict, this_prefix):
    ...
    if this_prefix not in prefix_dict:
        return 0
...
def Generate_Text(prefix_length, max_length, prefix_dict):
    ...
    while True:
        ...
        if not next_word:
            break  # We failed to find a suffix for the prefix
    ...
    if "." not in text:
        return Generate_Text(prefix_length, max_length - prefix_length, prefix_dict)
...
essay_list = [
    s.strip() for s in open(corpus_path, "r").read().split("\n" * 10) if len(s.strip())
]
...
try:
    prefix_length = int(input("Markov chunk size: "))
    if prefix_length < 1:
        raise Exception()
except:
...
try:
    words_length = int(input("Max word count: "))
    if prefix_length < 1:
        raise Exception()
except:
